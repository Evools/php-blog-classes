...
$conn = $db->getConnect();
$regisetrUser = new RegisterController($conn);

$err_user = $err_email = $err_pass = $err_confirm = "";
...
if(isset($_POST['register'])){
    ...
    if(empty($username)){
        $err_user = "Введите имя пользователя";
    }
    if(empty($email)){
    ...
    if(empty($password)){
        ...
        $err_pass = "Пароль должен быть больше 3 символов";
    }elseif ($password != $confirm){
        $err_pass = "Пароли не совпадают";
    }
    if(empty($confirm)){
        $err_confirm = "Подтвердите пароль";
    } elseif ($password != $confirm){
        $err_confirm = "Пароли не совпадают";
    }
    if( empty($err_user) && empty($err_email) && empty($err_pass) && empty($err_confirm)){

        $registrationResult = $regisetrUser->register_user($username, $email, $password_hash); // Вызываем метод и сохраняем результат

        if(!$registrationResult["success"]){
            $err_message = $registrationResult["message"];
        } else{
            $succes_message = "Вы успешно зарегистрировались | <a href='/signin' class='underline text-gray-800'>Войти</a>";
            $username = $email = $password = $confirm = "";
        }
    }
}
        ...
        <div class="max-w-md text-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="524.67004" height="531.39694" class="w-full" alt="https://undraw.co/illustrations" title="https://undraw.co/illustrations" viewBox="0 0 524.67004 531.39694" xmlns:xlink="http://www.w3.org/1999/xlink">
                ...
                <g>
                    ...
                    <path d="M266.02429,502.14508l5.42841-10.25366,15.68207,10.85684s3.61893,13.26944-1.80948,15.07895-20.50735-.60315-20.50735-.60315c0,0-27.74522,6.03156-28.34837,.60315s12.66631-6.63474,12.66631-6.63474l16.8884-9.04736,.00003-.00003Z" fill="#2f2e41" />
                    <path d="M325.13367,494.9072l-41.91943-26.41306-21.41208,19.17517-14.47577-16.8884s26.53891-39.2052,34.37994-41.01468,20.50735-3.01578,20.50735-3.01578l38.60205,35.58627-15.68207,32.5705v-.00003Z" fill="#2f2e41" />
                    <path d="M224.40645,491.28824l13.8726,15.07892-1.82944,1.56488s-3.59898,15.67496-12.04318,10.37241-9.04736-6.50888-9.04736-6.50888c0,0-24.12628,5.42844-26.53891-4.22211s4.22211-7.84103,4.22211-7.84103l6.02444,3.61893,25.33972-12.06314,.00002,.00003Z" fill="#2f2e41" />
                    <circle cx="383.03677" cy="361.00632" r="19.30103" fill="#ffb6b6" />
                    <path d="M378.51309,388.44998l27.74524-8.44421s10.85684,25.33261,9.04736,36.79257c-1.80948,11.45999-18.29459,66.05756-18.29459,66.05756l-36.59271-20.21765s-.60315-12.06314-2.41263-17.49155-7.53946-22.01523,5.12683-35.88785,15.38052-20.80893,15.38052-20.80893l-.00003,.00006Z" fill="#e6e6e6" />
                    <g>
                        <path id="uuid-375aaccb-22f9-42b6-a352-baabb12db8d2-179" d="M317.44769,515.10181c-4.51538,5.40955-5.38428,12.12427-1.9411,14.99738,3.44315,2.87305,9.89352,.81635,14.40979-4.59576,1.83371-2.13715,3.12897-4.68237,3.77734-7.42273l18.84238-23.1622-9.17844-7.42783-19.28064,22.56531c-2.58105,1.12839-4.85428,2.85858-6.62936,5.0459l.00003-.00006Z" fill="#ffb6b6" />
                        <polygon points="399.79492 400.61868 406.35291 426.59497 374.25586 477.97375 341.48965 510.90475 330.86948 502.09406 363.91479 460.97745 388.96295 407.99997 399.79492 400.61868" fill="#e6e6e6" />
                    </g>
                    <path d="M365.24362,357.68896c4.94998-.01184,5.23102,.02112,6.03156,0,6.1395-.1619,7.74496-2.57733,10.85684-2.41263,5.55417,.29398-1.77649,28.14084-4.82526,41.01468-2.99002,12.62589,7.11493,23.9397,7.84103,24.72943,10.25668,11.15479,28.27277,13.19449,31.36417,8.44421,2.50223-3.84491-6.36569-9.69107-4.22211-18.0947,2.09543-8.21484,11.77112-7.34009,14.47577-15.07892,2.83246-8.10455-6.68613-12.19214-12.66629-31.96732-3.43442-11.35699-2.82687-13.47311-6.03156-20.50732-8.56613-18.80209-50.55359-10.79666-45.83994,6.63474,.9118,3.37189-1.51474,7.24872,3.01578,7.23788v-.00006Z" fill="#2f2e41" />
                </g>
                <g>
                    ...
                    <path d="M307.41605,27.36262h-77.72357c-1.91588,0-3.47462-1.55832-3.47462-3.47462s1.55875-3.47462,3.47462-3.47462h77.72357c1.91589,0,3.47461,1.55832,3.47461,3.47462s-1.55875,3.47462-3.47461,3.47462Z" fill="#e6e6e6" />
                    <path d="M228.38948,91.64306c-.71851,0-1.30298,.58448-1.30298,1.30298,0,.35035,.1353,.67439,.38087,.91361,.2477,.25364,.57217,.38937,.9221,.38937h194.99193c.71851,0,1.30298-.58448,1.30298-1.30298,0-.35035-.13531-.67439-.38089-.91361-.24771-.25364-.57217-.38937-.92209-.38937H228.38948Z" fill="#e6e6e6" />
                    <path d="M332.1727,91.20873v3.47462h-103.78322c-.47775,0-.91208-.19113-1.22478-.51253-.32146-.3127-.51253-.74703-.51253-1.22478,0-.95555,.78181-1.73731,1.7373-1.73731h103.78322Z" fill="#000000" />
                    <path d="M421.20978,84.69383h-17.37308c-1.91589,0-3.47461-1.55832-3.47461-3.47462s1.55875-3.47462,3.47461-3.47462h17.37308c1.91589,0,3.47461,1.55832,3.47461,3.47462s-1.55875,3.47462-3.47461,3.47462Z" fill="#e6e6e6" />
                </g>
            </svg>
        </div>
    </div>
    ...
    <div class="w-full bg-gray-100 lg:w-1/2 flex items-center justify-center">
        <div class="max-w-md w-full p-6">
            <?php if($succes_message) : ?>
                <p class="p-3 rounded-lg bg-green-100 text-green-600 font-medium mb-3 flex items-center gap-2 justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                    <path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z" clip-rule="evenodd" />
                </svg>
                <span><?= $succes_message; ?></span>
            </p>
            <?php endif; ?>
            <?php if($err_message) : ?>
                <p class="p-3 rounded-lg bg-red-100 text-red-600 font-medium mb-3 flex items-center gap-2 justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
            ...
            <?php endif; ?>
            <h1 class="text-3xl font-semibold mb-6 text-black text-center">Регистрация</h1>
            <h1 class="text-sm font-semibold mb-6 text-gray-500 text-center">Присоединяйтесь к нашему сообществу бесплатно</h1>
            ...
            <form action="" method="POST" class="space-y-4">
                <div>
                    ...
                    <input value="<?= $username ?>" type="text" id="username" name="username" class="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300">
                    <?php if($err_user) : ?>
                        <small class="text-red-500"> <?= $err_user; ?> </small>
                    <?php endif; ?>
                </div>
                <div>
                    <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                    <input value="<?= $email ?>" type="text" id="email" name="email" class="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300">
                    <?php if($err_email) : ?>
                ...
                <div>
                    ...
                    <input value="<?= $password ?>" type="password" id="password" name="password" class="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300">
                    <?php if($err_pass) : ?>
                        <small class="text-red-500"> <?= $err_pass; ?> </small>
                    <?php endif; ?>
                </div>
                <div>
                    <label for="confirm" class="block text-sm font-medium text-gray-700">Повторить пароль</label>
                    <input value="<?= $confirm ?>" type="password" id="confirm" name="confirm" class="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300">
                    <?php if($err_confirm) : ?>
                        <small class="text-red-500"> <?= $err_confirm; ?> </small>
                    <?php endif; ?>
                </div>
                <div>
                    <button type="submit" name="register" class="w-full bg-black text-white p-2 rounded-md hover:bg-gray-800 focus:outline-none focus:bg-black focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 transition-colors duration-300">Зарегистрироваться</button>
                </div>
            </form>
            <div class="mt-4 text-sm text-gray-600 text-center">
                <p>У вас уже есть аккаунт? <a href="/signin" class="text-black hover:underline">Войти</a>
                </p>
            </div>
        </div>
    </div>
